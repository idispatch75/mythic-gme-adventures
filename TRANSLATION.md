...
The easiest way is to use the [Web version](https://mythic-gme-adventures.idispatch.ovh/) and then `Custom Meaning Tables` > `Import tables` in the Adventures List page. Then you enter an Adventure, select your language in the `Global Settings`, and `Enable Physical Dice Mode` to have a better view of all the words.
